...
class DataCenter(Topo):
    ...
    def __init__(self, n=1, delay='0ms', fi=1,  cpu=.01, max_queue_size=None, **params):
        ...
        levelLow = []
        Hosts = []
        # Nested for-loops are annoying.
        # Better to create and connect each level at the same time
        # Otherwise looping has to be done twice.
        for i in range (0, fi): #Fan-in rate
            # Create Mid-level switch
            levelMid.append(self.addSwitch("mls{}".format(i+1)))
            # Connect Top-level to Mid-level
            self.addLink(tls, levelMid[i], **swlinkConfig)
            for j in range(0, fi): # Fan-In rate
                # Create Low-level switch
                levelLow.append(self.addSwitch("s{}x{}".format(i+1,j+1)))
                # Conect Mid-level to Low-level
                self.addLink(levelMid[i], levelLow[(2*i)+j], **swlinkConfig)
                for k in range (0, n): # range is Num of Hosts
                    # Create Host
                    Hosts.append(self.addHost("h{}x{}x{}".format(i+1,j+1,k+1)))
                    # Connect Low-level to Hosts
                    self.addLink(levelLow[(2*i)+j], Hosts[(4*i)+(2*j)+k], **hostlinkConfig)
