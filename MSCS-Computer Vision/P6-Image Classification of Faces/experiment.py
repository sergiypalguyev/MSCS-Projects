...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    face = x_mean.reshape(size)
    cv2.normalize(face, face, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
    
    return cv2.resize(face, new_dims)
...
def helper_func(method, dataset):
    check=0
    for i in range(len(method)):
        if method[i] == dataset[i]:
            check += 1
...
def part_4_a_b():
    ...
    vj_accuracy = None
    vj_accuracy = 100. * helper_func(labels, predictions_train) / len(labels)
    ...
    vj_accuracy = None
    vj_accuracy = 100. * helper_func(real_labels, predictions_train) / len(real_labels)
