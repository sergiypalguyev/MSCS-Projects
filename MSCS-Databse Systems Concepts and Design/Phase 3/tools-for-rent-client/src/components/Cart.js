...
export default class Cart extends React.Component{
   ...
   submitConfirmation(){
       ...
       let tools = [];
       JSON.parse(localStorage.getItem("toolsAddedToCart")).forEach(function(item){
          tools.push(item.id);
       });
    ...
    let endDate = end.getFullYear() + '-' +
       ...
       axios.post('http://localhost:8080/reservation/make', {
           userID,
           startDate,
           endDate,
           tools
         ...
         .then((result) => {
           console.log(result);
           localStorage.setItem("reservationID", result.data.errorCode);
           this.props.history.push('/reservationConfirmation');
    ...
    removeItems(){
        let storedItems = JSON.parse(localStorage.getItem("toolsAddedToCart"));
        this.state.selectedItems.forEach(function(item){
            let idToBeRemoved = (JSON.parse(localStorage.getItem("toolsAddedToCart")))[item].id;
            storedItems.forEach(function (sItem, index, object){
                if(sItem.id == idToBeRemoved){
                    object.splice(index, 1);
                }
            });
        });
        localStorage.setItem("toolsAddedToCart", JSON.stringify(storedItems));
    ...
    render(){
        ...
        return(
            ...
            {!isCartEmpty &&
             <div>
             <NavBar/>
            <h2 ref={subtitle => this.subtitle = subtitle}>Reservation Summary</h2>
             ...
             <hr size="10"/>
              ...
              <RaisedButton label="Submit" secondary={true} style={style} onClick={this.submitConfirmation} />
              <RaisedButton label="Reset" secondary={true} style={style} onClick={this.resetLocalStorage}/>
              <RaisedButton label="Remove Selected Items" secondary={true} style={style} onClick={this.removeItems}/>
