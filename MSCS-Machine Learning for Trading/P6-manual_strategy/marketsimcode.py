...
def compute_portvals(trades, start_val = 1000000, commission=9.95, impact=0.005):  		   	  			    		  		  		    	 		 		   		 		  
    ...
    # Sort DataFrame
    trades = trades.sort_values([str_Date])
    
    # Get company ticker symbols
    # Get Date Range
    # Get Prices
    tickers = list(trades[str_Symbol].unique())
    dates = pd.date_range(trades[str_Date].iloc[0],trades[str_Date].iloc[-1])
    ...
    for order in trades.iterrows():
        ...
        if order[1][2] == str_Buy:
            ...
            df_portfolio.loc[order[1][0]:, order[1][1]] -= order[1][3]
            df_portfolio.loc[order[1][0]:, str_Cash] = df_portfolio.loc[order[1][0], str_Cash]
